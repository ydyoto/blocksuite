...
import { BlockElementModel } from './edgeless-block-model.js';
...
export class EdgelessRootService extends RootService {
  ...
  removeElement(id: string | BlockSuite.EdgelessModelType) {
    ...
    if (el instanceof BlockElementModel) {
  ...
  reorderElement(
    ...
    if (element instanceof BlockElementModel) {
      this.doc.transact(() => {
        element.index = index;
      });
    } else {
  ...
  get blocks() {
    return (this.frames as BlockElementModel[]).concat(this._layer.blocks);
  }
