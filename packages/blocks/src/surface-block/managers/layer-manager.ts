...
import { BlockElementModel } from '../../root-block/edgeless/edgeless-block-model.js';
import {
  SurfaceElementModel,
...
export type BlockLayer = BaseLayer<BlockElementModel> & {
  type: 'block';
...
export class LayerManager {
  ...
  blocks!: BlockElementModel[];

  blocksGrid = new GridManager<BlockElementModel>();
    ...
    const layerManager = new LayerManager(
      (
        doc
          ...
          .filter(
            model =>
              model instanceof BlockElementModel &&
  ...
  private _updateLayer(
    ...
    if (!type.startsWith('affine:')) {
      ...
      this.blocksGrid.update(element as BlockElementModel);
    }
    ...
    this._disposables.add(
      doc.slots.blockUpdated.on(payload => {
        if (payload.type === 'add') {
          ...
          if (
            block instanceof BlockElementModel &&
            renderableInEdgeless(doc, surface, block) &&
            this.blocks.indexOf(block) === -1
          ) {
            this.add(block as BlockElementModel);
          }
        }
        if (payload.type === 'update') {
          ...
          if (
            ...
            this.update(block as BlockElementModel, {
        ...
        if (payload.type === 'delete') {
          ...
          if (block instanceof BlockElementModel) {
            this.delete(block as BlockElementModel);
          }
        }
      })
    );
  ...
  add(element: BlockSuite.EdgelessModelType) {
    ...
    if (!type.startsWith('affine:')) {
      ...
      this.blocksGrid.add(element as BlockElementModel);
    }
