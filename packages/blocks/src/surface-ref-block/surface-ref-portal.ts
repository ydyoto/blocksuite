...
export class SurfaceRefPortal extends WithDisposable(ShadowlessElement) {
  private _getBlocksInFrame = (model: FrameBlockModel): BlockElementModel[] => {
  ...
  private _getBlocksInGroup(model: GroupElementModel): BlockElementModel[] {
    return Array.from(model.childIds)
      .map(id => this.doc.getBlockById(id) as BlockElementModel)
      .filter(el => el);
  }
